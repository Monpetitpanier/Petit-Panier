...
export default function DetailsChargeFixe() {
  const navigation = useNavigation();
  const route = useRoute();

  const { chargeId } = route.params;

  const { chargesFixes, supprimerChargeFixe } = useBudget();

  const charge = chargesFixes.find((item) => item.id === chargeId);
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        <Text style={styles.sousTitre}>Charge fixe</Text>
        ...
        {charge.jour && (
          <View style={styles.carte}>
            ...
            <Text style={styles.valeur}>Le {charge.jour} du mois</Text>
          </View>
        )}
